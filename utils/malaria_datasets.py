...
# MALARIA_URL = "https://deepchemdata.s3-us-west-1.amazonaws.com/datasets/delaney-processed.csv"
MALARIA_TASKS = ['activity']

MALARIA_FILE = "../test_data/malaria.csv"

class _MalariaLoader(_MolnetLoader):

  def create_dataset(self) -> Dataset:
    # dataset_file = os.path.join(self.data_dir, "delaney-processed.csv")
    dataset_file = MALARIA_FILE
    if not os.path.exists(dataset_file):
      dc.utils.data_utils.download_url(url=MALARIA_FILE, dest_dir=self.data_dir)
    # loader = CSVLoader(
...
def load_malaria(
  ...
  """
  loader = _MalariaLoader(featurizer, splitter, transformers, MALARIA_TASKS,
