...
# CEP_URL = "https://deepchemdata.s3-us-west-1.amazonaws.com/datasets/delaney-processed.csv"
CEP_TASKS = ['PCE']

CEP_FILE = "../test_data/cep.csv"

class _CepLoader(_MolnetLoader):

  def create_dataset(self) -> Dataset:
    # dataset_file = os.path.join(self.data_dir, "delaney-processed.csv")
    dataset_file = CEP_FILE
    if not os.path.exists(dataset_file):
      dc.utils.data_utils.download_url(url=CEP_FILE, dest_dir=self.data_dir)
    # loader = CSVLoader(
...
def load_cep(
  ...
  """
  loader = _CepLoader(featurizer, splitter, transformers, CEP_TASKS,
