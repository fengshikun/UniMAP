...
def get_self_loops_typeid():
    return allowable_features['possible_bond_type_list'].index('[SELF]')


def get_mask_atom_typeid():
    return allowable_features["possible_atomic_num_list"].index('[MASK]')


def get_mask_edge_typeid():
    return allowable_features['possible_bond_type_list'].index('[MASK]')


def get_mask_atom_feature():
    atom_feature = [
        safe_index(allowable_features['possible_atomic_num_list'], '[MASK]'),
        allowable_features['possible_chirality_list'].index('CHI_UNSPECIFIED'),
        safe_index(allowable_features['possible_degree_list'], 'misc'),
        safe_index(allowable_features['possible_formal_charge_list'], 'misc'),
        safe_index(allowable_features['possible_numH_list'], 'misc'),
        safe_index(allowable_features['possible_number_radical_e_list'], 'misc'),
        safe_index(allowable_features['possible_hybridization_list'], 'misc'),
        allowable_features['possible_is_aromatic_list'].index(False),
        allowable_features['possible_is_in_ring_list'].index(False),
    ]
    return atom_feature
...
def get_bond_mask_feature():
    bond_feature = [
        safe_index(allowable_features['possible_bond_type_list'], '[MASK]'),
        safe_index(allowable_features['possible_bond_stereo_list'], '[MASK]'),
