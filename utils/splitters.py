...
class ScaffoldSplitter(Splitter):
  ...
  def split(self,
    ...
    val_size, test_size = frac_valid * len(dataset), frac_test * len(dataset)
    ...
    # new_scaffold_sets = big_index_sets + small_index_sets
  ...
  def generate_scaffolds(self, dataset: Dataset,
    ...
    if dataset.ids.shape[1] == 2: # contains iupac
        smiles_array = dataset.ids[:, 0]
    else:
        smiles_array = dataset.ids
    
    for ind, smiles in enumerate(smiles_array):
...
class ScaffoldBalanceSplitter(Splitter):
  """Class for doing data splits based on the scaffold of small molecules.
  ...
  def split(self,
    ...
    random.shuffle(big_index_sets)
    random.shuffle(small_index_sets)
    balance_scaffold_sets = big_index_sets + small_index_sets
    
    
    
    
    for scaffold_set in balance_scaffold_sets:
      if len(train_inds) + len(scaffold_set) > train_cutoff:
        if len(train_inds) + len(valid_inds) + len(scaffold_set) > valid_cutoff:
          test_inds += scaffold_set
        else:
