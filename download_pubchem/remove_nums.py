...
if __name__=='__main__':
    ...
    pattern = r'[0-9]'
    data['no_nums'] = data['Preferred'].apply(lambda x: re.sub(pattern,'',x)).apply(lambda x:x.replace(',','')).apply(lambda x:x.replace('-',' '))

    data.drop(['Preferred'],axis=1,inplace=True)
    # data.drop(['no_bracs'],axis=1,inplace=True)
    
    print('Writing...')
    data.to_csv('/sharefs//chem_data/pubchem/data_1m/iupacs_no_nums.csv',index=False)