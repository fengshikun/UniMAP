...
# python finetune.py --datasets=bbbp --pretrained_model_name_or_path=/sharefs//chem_data/results/output_dir/debug/checkpoint-6578
...
# gcn parameters
flags.DEFINE_integer(name="gnn_number_layer", default=12, help="")
...
flags.DEFINE_integer(name="early_stopping_patience", default=8, help="")
    ...
    for i in range(len(FLAGS.datasets)):
        ...
        if dataset_name != 'deepddi':
            ...
            for seed in seed_lst:
                ...
                dataset_name = "{}_seed{}".format(dataset_name, seed)
...
def finetune_single_dataset(dataset_name, dataset_type, run_dir, is_molnet, seed=0):
    ...
    if dataset_name == 'deepddi':
        ...
        finetune_datasets = get_finetune_datasets(dataset_name, tokenizer, is_molnet, seed=seed)
    ...
    class CustomCallback(TrainerCallback):
        ...
        def get_topk_best_test_res(self, topk=5):
    ...
    def custom_hp_space_optuna(trial):
        return {
            ...
            "per_device_train_batch_size": trial.suggest_categorical(
                "per_device_train_batch_size", [64]#64,128,256
            ),
            "weight_decay": trial.suggest_float("weight_decay", 1e-7, 1e-3, log=True),
        }

    dir_valid = os.path.join(run_dir, "results", "valid")
    dir_test = os.path.join(run_dir, "results", "valid")
    os.makedirs(dir_valid, exist_ok=True)
    os.makedirs(dir_test, exist_ok=True)
    random_seed = 0
    test_debug_res = eval_model(
            trainer,
            finetune_datasets.test_dataset,
            dataset_name,
            dataset_type,
            ...
            random_seed,
            finetune_datasets.num_tasks,
            finetune_datasets.train_dataset
        )
    val_debug_res = eval_model(
            trainer,
            finetune_datasets.valid_dataset_unlabeled,
            dataset_name,
            dataset_type,
    ...
    print(f'test res is {test_debug_res}, val res is {val_debug_res}')
    ...
    for random_seed in range(FLAGS.n_seeds):
        ...
        dummy_hyperparameters = {'learning_rate': 3e-05, 'per_device_train_batch_size': 32, 'weight_decay': 1e-06}
        print("Search result is: {}".format(dummy_hyperparameters))


        for n, v in dummy_hyperparameters.items():
            setattr(trainer.args, n, v)

        trainer.args.num_train_epochs = 100

        if FLAGS.datasets[0] == 'deepddi':
            trainer.args.num_train_epochs = 60
    ...
    if dataset_type == "classification":
        if len(np.unique(labels)) <= 2:
            ...
            if num_tasks > 1:
                ...
                # debug:
                # train_label_ratio =  train_dataset.labels.sum(axis=0) / train_dataset.labels.shape[0]
                # test_label_ratio = dataset.labels.sum(axis=0) / dataset.labels.shape[0]
                # for i in range(num_tasks):
                #     print("Task {}: {}\t {} \t {}".format(i, results[i], train_label_ratio[i], test_label_ratio[i]))

                
                
                # y_pred = []
                # for pred in predictions.predictions:
                #     y_pred.append(softmax(pred, axis=1)[:, 1])
            ...
            f1_macro = f1_score(labels,y_pred,average='macro') #macro-F1
            prec_macro = precision_score(labels,y_pred,average='macro') #macro-Precision
            rec_macro = recall_score(labels,y_pred,average='macro') #macro-Recall

            metrics = {"mcc": matthews_corrcoef(labels, y_pred), 'acc_score': accuracy_score(labels, y_pred), 'f1_macro':f1_macro,'prec_micro':prec_macro,'rec_micro':rec_macro}
...
def get_finetune_datasets(dataset_name, tokenizer, is_molnet, seed=0):
    if is_molnet:
    ...
    train_dataset = FinetuneDataset(train_df, tokenizer, use_struct_pos=FLAGS.use_struct, tasks_wanted=tasks_wanted, iupac_only=FLAGS.iupac_only, lang_only=FLAGS.lang_only, gnn_only=FLAGS.gnn_only, iupac_smiles_concat=FLAGS.iupac_smiles_concat, graph_uni=FLAGS.graph_uni, use_rdkit_feature=FLAGS.use_rdkit_feature)
    valid_dataset = FinetuneDataset(valid_df, tokenizer, use_struct_pos=FLAGS.use_struct, tasks_wanted=tasks_wanted, iupac_only=FLAGS.iupac_only, lang_only=FLAGS.lang_only, gnn_only=FLAGS.gnn_only, iupac_smiles_concat=FLAGS.iupac_smiles_concat, graph_uni=FLAGS.graph_uni, use_rdkit_feature=FLAGS.use_rdkit_feature)
    ...
    valid_dataset_unlabeled = FinetuneDataset(valid_df, tokenizer, include_labels=False, use_struct_pos=FLAGS.use_struct, tasks_wanted=tasks_wanted, iupac_only=FLAGS.iupac_only, lang_only=FLAGS.lang_only, gnn_only=FLAGS.gnn_only, iupac_smiles_concat=FLAGS.iupac_smiles_concat, graph_uni=FLAGS.graph_uni, use_rdkit_feature=FLAGS.use_rdkit_feature)
