...
class InputExample:
    ...
    """
    labels: Optional[List[int]] = None # the atom numbers labels
    ...
    class ParsedSeqDataset(Dataset):# ParsedIUPACDataset
        ...
        def __init__(
            ...
            if max_pos_depth is None or 'vanilla' in task:
                ...
                data_dir,
                "cached_fg_formula_{}_{}_{}_{}_{}".format(
                    mode.value,
                ...
                cached_features_file = os.path.join(
                    data_dir,
                    "cached_fg_formula_{}_{}_{}_{}_{}_{}".format(
                        mode.value,
                        tokenizer.__class__.__name__,
                        tokenizer.vocab_size,
                        str(max_seq_length),
    ...
    def _create_examples(self, lines: List[List[str]], type: str):
        ...
        examples = [
            InputExample(
                ...
                labels=list(line['atom_number'].values()),
            )
            for line in json_lines 
        ]
...
def convert_examples_seq_to_features(
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        if ex_index < 5:
            ...
            logger.info("reg_labels: %s", " ".join([str(x) for x in labels]))
